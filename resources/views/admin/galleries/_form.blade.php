...
<div class="mb-6">
    ...
    <input type="text" id="title" name="title" value="{{ old('title', $gallery->title ?? '') }}"
        class="shadow appearance-none border @error('title') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500"
        required autofocus>
    @error('title')
...
<div class="mb-6">
    ...
    <select id="category" name="category"
        class="shadow appearance-none border @error('category') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500"
        required>
    ...
    @if (isset($gallery) && $gallery->image_path)
        <div class="mt-4">
            ...
            <img src="{{ asset('storage/' . $gallery->image_path) }}" alt="{{ $gallery->title }}"
                class="w-full h-auto max-w-sm mt-2 rounded-md shadow-md">
        </div>
    @endif
</div>
